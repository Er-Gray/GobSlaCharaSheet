...
function App() {
  ...
  useEffect(() => {
    const charLengthSet = (adOrGe) => {
      let maxLen = 0;
      const countCharLen = (value) => {
        value.forEach((element) => {
          let len = 0;
          Array.prototype.forEach.call(element, (s) => {
            if (s.match(/[ -~]/)) {
              len += 1;
            } else {
              len += 2;
      ...
      const insertSpace = (value) => {
        const spacedValue = value.map((element) => {
          let spacedElement = "";
      ...
      if (race.name) {
        countCharLen(race[adOrGe]);
        countCharLen(histories.出自[adOrGe]);
        countCharLen(histories.来歴[adOrGe]);
        setRace({ ...race, [adOrGe]: insertSpace(race[adOrGe]) });
        setHistories({
          ...histories,
          出自: {
            ...histories.出自,
            [adOrGe]: insertSpace(histories.出自[adOrGe]),
          },
        });
        setHistories({
          ...histories,
          来歴: {
            ...histories.来歴,
            [adOrGe]: insertSpace(histories.来歴[adOrGe]),
          },
        });
      }
      if (histories.出自[adOrGe]) {
      }
      if (histories.来歴[adOrGe]) {
      }
    };
    charLengthSet("adventureSkill");
    charLengthSet("generalSkill");
  }, [race, histories]);
      ...
      <div id="rightArea">
        ...
        <Context.Provider
          value={{
            ...
            setFirstStatus: setFirstStatus,
            rolled: rolled,
            bonusAdeed: bonusAdeed,
            setBonusAdded: setBonusAdded,
          }}
        >
          <AddBonusArea />
        </Context.Provider>
        <Context.Provider
          value={{ stateValues: stateValues, setStateValues: setStateValues }}
        >
          <StateValueFixedArea />
        </Context.Provider>
        <Context.Provider
        ...
        <Context.Provider
          value={{ dayWalker: dayWalker, setDayWalker: setDayWalker }}
        >
          <UseDayWalker />
        </Context.Provider>
      </div>
...
function DecisionRaceArea() {
  ...
  const { race, setRace, histories, setHistories, coin, setCoin } =
    useContext(Context);
  ...
  const raceChoices = (beastBindIn) => {
    if (beastBindIn) {
      return Object.keys(races.種族).map((viewRace, index) => (
        <option key={index} value={viewRace}>
          {viewRace}
        </option>
      ));
    } else {
      return Object.keys(races.種族)
        .map((viewRace, index) => (
          <option key={index} value={viewRace}>
            {viewRace}
          </option>
        ))
        .filter((elm) => elm.props.value !== "獣憑き");
    }
  };
  ...
  useEffect(() => {
    if (
      ...
      setHistories({
        出自: table.出自[originalRace.replace(/\(.*\)/, "")][
          oneDSix() + oneDSix() - 2
        ],
        来歴: table.来歴[oneDSix() + oneDSix() - 2],
        邂逅: table.邂逅[oneDSix() + oneDSix() - 2],
      });
    }
...
function DecisionFirstAndSecondStatusArea() {
  const [firstStatusRoll, setFirstStatusRoll] = useState({
    体力点: 0,
    魂魄点: 0,
    技量点: 0,
    知力点: 0,
  });
  const [secondStatusRoll, setSecondStatusRoll] = useState({
    集中度: 0,
    持久度: 0,
    反射度: 0,
  });
  ...
  const { register, handleSubmit, errors } = useForm();
  const { setFirstStatus, setSecondStatus, race, setRolled } =
    useContext(Context);
  ...
  const randomOrFixedSubmit = (data) => {
    ...
    if (data.randomOrFixed === "random") {
      ...
      if (!race.name) {
        ...
        setFirstStatus({
          体力点: statusTable.固定値[race.name][0].体力点,
          魂魄点: statusTable.固定値[race.name][0].魂魄点,
          技量点: statusTable.固定値[race.name][0].技量点,
          知力点: statusTable.固定値[race.name][0].知力点,
        });
        setSecondStatus({
          集中度: statusTable.固定値[race.name][1].集中度,
          持久度: statusTable.固定値[race.name][1].持久度,
          反射度: statusTable.固定値[race.name][1].反射度,
        });
        setFirstStatusRoll({
          体力点: statusTable.固定値[race.name][0].体力点,
          魂魄点: statusTable.固定値[race.name][0].魂魄点,
  ...
  if (randomStatus && race.name) {
    setFirstStatus({
      体力点:
        firstStatusRoll.体力点 + statusTable.ランダム修正[race.name][0].体力点,
      魂魄点:
        firstStatusRoll.魂魄点 + statusTable.ランダム修正[race.name][0].魂魄点,
      技量点:
        firstStatusRoll.技量点 + statusTable.ランダム修正[race.name][0].技量点,
      知力点:
        firstStatusRoll.知力点 + statusTable.ランダム修正[race.name][0].知力点,
    });
    setSecondStatus({
      集中度:
        secondStatusRoll.集中度 + statusTable.ランダム修正[race.name][1].集中度,
      持久度:
        secondStatusRoll.持久度 + statusTable.ランダム修正[race.name][1].持久度,
      反射度:
        secondStatusRoll.反射度 + statusTable.ランダム修正[race.name][1].反射度,
    });
  ...
  if (!firstStatusRoll.体力点 && !secondStatusRoll.集中度) {
    ...
    DecisionFirstAndSecondStatusArea = (
      <div className="area">
        ...
        <h3>第一能力値</h3>
        <p>
          体力点:{firstStatusRoll.体力点} 魂魄点:{firstStatusRoll.魂魄点}{" "}
          技量点:{firstStatusRoll.技量点} 知力点:{firstStatusRoll.知力点}
        </p>
        <h3>第二能力値</h3>
        <p>
          集中度:{secondStatusRoll.集中度} 持久度:{secondStatusRoll.持久度}{" "}
          反射度:{secondStatusRoll.反射度}
        </p>
      </div>
    );
  }
...
function ReliefStatusArea() {
  ...
  const sumStatus = () => {
    let sum = 0;
    Object.keys(firstStatus).forEach((key) => {
      sum += firstStatus[key];
    });
    Object.keys(secondStatus).forEach((key) => {
      sum += secondStatus[key];
    });
    return sum;
  };
  ...
  const reliefSubmit = (data) => {
    ...
    if (firstStatus[data.decided] !== undefined) {
      setFirstStatus({
        ...firstStatus,
        [data.decided]:
          3 + statusTable.ランダム修正[race.name][0][data.decided],
      });
    } else if (secondStatus[data.decided] !== undefined) {
      setSecondStatus({
        ...secondStatus,
        [data.decided]:
          3 + statusTable.ランダム修正[race.name][1][data.decided],
      });
    }
    ...
    reliefStatusArea = (
      <div className="area">
        ...
        <form onSubmit={handleSubmit(reliefSubmit)}>
          <p>
            能力値合計が15以下なので好きな能力値の出目を3がでたことにできます
          </p>
...
function AddBonusArea() {
  const { firstStatus, setFirstStatus, rolled, bonusAdeed, setBonusAdded } =
    useContext(Context);
  const { register, handleSubmit, errors } = useForm();
  let addBonusArea = "";
  ...
  const bonusSubmit = (data) => {
    ...
    setFirstStatus({
      ...firstStatus,
      [data.decided]: firstStatus[data.decided] + 1,
    });
    setBonusAdded(true);
  };
    ...
    stateValueFixedArea = (
      <div className="area">
        ...
        <form onSubmit={handleSubmit(stateValueSubmit)}>
          <p>状態値に固定値を使いますか？</p>
          <button type="submit" name="fixed" value="true" ref={register}>
            使う
          </button>
        </form>
      </div>
...
function VitalityDicisionArea() {
  ...
  const valueChoice = () => {
    return stateValues.map((value, index) => (
  ...
  const vitalitySubmit = (data) => {
    ...
    if (data.decided) {
      ...
      newValues.splice(
        newValues.findIndex((element) => element === parseInt(data.decided)),
        1
      );
...
function MovilityDecisionArea() {
  const { register, handleSubmit, errors } = useForm();
  const { status, setStatus, stateValues, setStateValues, race } =
    useContext(Context);
...
function SpellCountDecisionArea() {
  const { register, handleSubmit, errors } = useForm();
  const { status, setStatus, stateValues, setStateValues } =
    useContext(Context);
...
function UseDayWalker() {
  ...
  const valueChoice = () => {
    return Object.keys(dayWalkerTable).map((value, index) => (
      <option key={index} value={value}>
        {value}
      </option>
    ));
